...
class KANG_MultiTask(nn.Module):
	def __init__(self,
			in_channels, 
			hidden_channels, 
			num_tasks,
		...
		super(KANG_MultiTask, self).__init__()
		...
		# Multi-task classification output heads
		...
		for _ in range(num_tasks):
			...
			if kan:
				...
				self.task_heads.append(MLP([final_input_dim, out_dim]))
	...
	def forward(self, x, edge_index, batch=None, global_features=None, edge_attr=None):
		...
		# Add self-loops and extend edge_attr
		...
		# 4. Multi-task classification outputs
		task_outputs = []
		for task_head in self.task_heads:
			task_out = task_head(x)
			# Apply log_softmax for classification tasks
			task_out = F.log_softmax(task_out, dim=1)
			task_outputs.append(task_out)
		
		# Stack outputs: [batch_size, num_tasks, 2]

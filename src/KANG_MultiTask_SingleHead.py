...
class KANG_MultiTask_SingleHead(nn.Module):
    def __init__(self,
            in_channels, 
            hidden_channels, 
            num_tasks,
        ...
        super(KANG_MultiTask_SingleHead, self).__init__()
        ...
        # Single head for all tasks: output shape [batch_size, num_tasks, 2]
        out_dim = 2
        if kan:
            if bsplines:
                self.head = KANLinear(final_input_dim, num_tasks * out_dim, grid_size=num_grids, grid_range=[grid_min, grid_max])
            else:
                self.head = KAND(final_input_dim, num_tasks * out_dim, grid_min, grid_max, num_grids, device=device, linspace=linspace, trainable_grid=trainable_grid)
        else:
            self.head = MLP([final_input_dim, num_tasks * out_dim])
    ...
    def forward(self, x, edge_index, batch=None, global_features=None, edge_attr=None):
        ...
        out = self.head(x)  # [batch_size, num_tasks * 2]
    ...
    def predict_single_task(self, x, edge_index, batch=None, global_features=None, task_idx=0, edge_attr=None):
        ...
        out = self.head(encoded)  # [batch_size, num_tasks * 2]
        out = out.view(-1, self.num_tasks, 2)
        task_output = out[:, task_idx, :]  # [batch_size, 2]
